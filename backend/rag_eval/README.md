...
如果你已经把 Neo4j 的 KG 检索接进了聊天主链，可以直接评测“文档检索 + KG 检索”的融合结果：

```bash
python backend/rag_eval/cli.py evaluate-fusion \
  --dataset backend/rag_eval/examples/fusion_eval_dataset.example.json \
  --custom-model-id YOUR_CUSTOM_MODEL_ID \
  --relevance-threshold 0.0 \
  --output backend/rag_eval/examples/fusion_eval_results.json
```

也可以不用 `custom-model-id`，直接指定知识库或 collection：
...
```json
{
  ...
  "relevant_kg_nodes": [
    { "node_id": "neo4j-id-001", "node_name": "电机过热", "labels": ["Fault"] },
    { "node_name": "冷却不良", "labels": ["Cause"] }
  ]
}
```
...
其中 `fusion` 的 gold 规则是：

- 如果这条 query 标了 `relevant_chunks`，文档侧 gold 用 chunk
- 如果没标 chunk，只标了 `relevant_files`，文档侧 gold 用 file
- 再把 `relevant_kg_nodes` 一起并入最终融合 gold
