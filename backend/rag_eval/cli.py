...
from rag_eval.fusion_evaluator import FusionRetrievalEvaluator  # noqa: E402
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    fusion_eval_parser = subparsers.add_parser(
        "evaluate-fusion",
        help="Run fused document + KG retrieval evaluation using a labeled dataset",
    )
    fusion_eval_parser.add_argument("--dataset", required=True, help="Path to the labeled evaluation dataset JSON")
    fusion_eval_parser.add_argument(
        "--output",
        default="",
        help="Optional output path. Defaults to <dataset>.fusion.results.json",
    )
    fusion_eval_parser.add_argument(
        "--config-name",
        default="development",
    ...
    fusion_eval_parser.add_argument(
        "--disable-kg",
        action="store_true",
        help="Disable KG retrieval and evaluate document-only results through the fusion evaluator",
    )
    fusion_eval_parser.add_argument(
        "--document-limit",
        type=int,
        default=None,
        help="Override the document-side candidate limit before fusion",
    )
    fusion_eval_parser.add_argument(
        "--kg-limit",
        type=int,
        default=None,
        help="Override the KG-side candidate limit before fusion",
    )
    fusion_eval_parser.add_argument(
        "--relevance-threshold",
        type=float,
...
def _default_fusion_output_path(dataset_path: str) -> Path:
    source = Path(dataset_path)
    if source.suffix.lower() == ".json":
        return source.with_suffix(".fusion.results.json")
    return source.parent / f"{source.name}.fusion.results.json"
...
def _command_evaluate_fusion(args: argparse.Namespace) -> int:
    if not args.kb_id and not args.collection and not args.custom_model_id:
        raise ValueError("Provide at least one of --kb-id, --collection, or --custom-model-id")
    ...
    with app.app_context():
        ...
        evaluator = FusionRetrievalEvaluator()
        result = evaluator.evaluate_dataset(
            app=app,
            dataset=dataset,
            collection_names=resolved["collection_names"],
    ...
    output_path = Path(args.output) if args.output else _default_fusion_output_path(args.dataset)
    write_json(output_path, result)

    print(f"Fusion evaluation written to: {output_path}")
    print(f"Collections: {', '.join(result['retrieval_settings']['collection_names'])}")
    print(f"Queries: {result['summary']['total_queries']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "evaluate-fusion":
        return _command_evaluate_fusion(args)
